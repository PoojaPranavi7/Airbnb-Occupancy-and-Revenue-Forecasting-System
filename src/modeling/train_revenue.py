...
def prepare(df):
    ...
    drop = {"date", "neighbourhood_cleansed", "occupancy_rate", "avg_price", "revenue", "avg_price_est", TARGET}
...
def main():
    ...
    # Ensure target exists and not null
    ...
    joblib.dump(model, MODELS_DIR / "lgbm_revenue.joblib")
    (MODELS_DIR / "revenue_features.json").write_text(json.dumps(feature_cols, indent=2))
    (REPORTS_DIR / "metrics_revenue.json").write_text(json.dumps(metrics, indent=2))

    print("Saved models/lgbm_revenue.joblib")
    print("Saved models/revenue_features.json")
    print("Saved reports/metrics_revenue.json")
    print(metrics)
